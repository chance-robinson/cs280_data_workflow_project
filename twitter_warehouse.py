...
def call_api(ti: TaskInstance, **kwargs):
    ...
    user_requests = [requests.get(f"https://api.twitter.com/2/users/{id[0]}?user.fields=public_metrics", headers=header_token).json() for id in users]
    user_tweet_requests = [requests.get(f"https://api.twitter.com/2/tweets/{id[0]}?tweet.fields=author_id,text,public_metrics", headers=header_token).json() for id in tweets]
    print(user_requests)
